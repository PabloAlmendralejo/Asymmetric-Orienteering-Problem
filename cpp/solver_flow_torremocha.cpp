...
static Input parse_input(const std::string& json_str) {
    ...
    // find_key: returns false (leaves i untouched) if the key is absent,
    // instead of throwing -- needed for optional fields (gain/loss/rho)
    // so older benchmark JSONs without them still parse.
...
// Mirrors the base-cost pre-filter in LPModel::build() (eq. arcelim in the
// paper) but does NOT use the fatigue-aware filter, since that filter
// itself depends on Ghat -- this would be circular. Using only the
// cost-only filter here is conservative (a superset of true survivors),
// which keeps Ghat a valid upper bound.
...
// Ĝ_i: valid upper bound on the (unclipped) cumulative fatigue state G_i
// reachable at node i by any budget-admissible route from the depot.
//
// Computed via a Bellman-Ford relaxation in the (max, +) semiring: instead
// of the usual shortest-path min-relaxation, we maximise, and we cap the
// number of rounds at n-1 (= the longest a *simple* path through n+1 nodes
// can be). This is the key point that makes the bound valid despite the
// arc graph having possible positive-weight cycles (e.g. oscillating
// between an uphill and downhill arc could in principle inflate G forever
// under a naive longest-path relaxation): by stopping at n-1 rounds we are
// only maximising over walks of length <= n-1, which is a superset of all
// simple paths and therefore still an upper bound on any simple-path G_i,
// while remaining well-defined (no infinite loop) regardless of cycle sign.
//
// Complexity: O(n^2) arcs x O(n) rounds = O(n^3), trivial for n <= 100.
static std::vector<double> compute_fatigue_bounds(const Input& inp, double rho) {
    ...
    // Precompute the (small) arc list once.
    std::vector<std::array<double,3>> arcs;  // {i, j, psi_ij}
    arcs.reserve(n * n);
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
...
// applies the true max(0, ·) reservoir dynamics from the corrected model:
//   F_0 = 0
//   F_{l+1} = max(0, F_l + phi_plus - rho*phi_minus)
...
struct LPModel {
    ...
    void build(const Input& inp) {
        ...
        // x[i][j] ΓÇö pre-fix structurally infeasible arcs
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
    ...
    // Propagation: sum_i (g_ij + psi_ij * x_ij) = sum_k g_jk, for j != 0.
    // psi_ij = phi_plus_ij - rho*phi_minus_ij can be negative (net downhill
    // recovery), unlike the pure-cost time propagation this is based on.
...
static void run_map(const std::string& in_path, const std::string& out_path,
    ...
    // Both models reported side by side: "fatigue_cost" is the corrected
    // (clipped, asymmetric) model now used for feasibility; "fatigue_cost_legacy"
    // is the old linear-in-elapsed-time model (Sec 3.6 as originally
    // submitted), kept only as a reference point for the paper's A/B story.
...
int main(int argc, char** argv) {
    ...
    {"op_input_torremocha_ring.json",          "op_output_torremocha_ring.json"},
    {"op_input_torremocha_path_biased.json",   "op_output_torremocha_path_biased.json"},
    {"op_input_torremocha_elev_biased.json",   "op_output_torremocha_elev_biased.json"},
    ...
    // collide with the sibling mtz solver's run over the same directory.
    std::string fixed_suffix;
    if (fixed_lambda >= 0.0 || fixed_rho >= 0.0) fixed_suffix = "_fixedLR_flow";
...
cmd /c ""C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\VC\Tools\MSVC\14.50.35717\bin\Hostx64\x64\cl.exe" /O2 /std:c++20 /EHsc op_bcn_with_highs_100nodes.cpp /I"C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\VC\Tools\MSVC\14.50.35717\include" /I"C:\Program Files (x86)\Windows Kits\10\Include\10.0.26100.0\ucrt" /I"C:\Program Files (x86)\Windows Kits\10\Include\10.0.26100.0\shared" /I"C:\Program Files (x86)\Windows Kits\10\Include\10.0.26100.0\um" /I"C:\Users\borrepa\Downloadsnz_dem_jsons\HiGHS\highs" /I"C:\Users\borrepa\Downloadsnz_dem_jsons\HiGHS\build" /link /LIBPATH:"C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\VC\Tools\MSVC\14.50.35717\lib\x64" /LIBPATH:"C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\ucrt\x64" /LIBPATH:"C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\um\x64" /LIBPATH:"C:\Users\borrepa\Downloadsnz_dem_jsons\HiGHS\build\Release\bin" highs.lib /out:op_bcn_with_highs_100nodes.exe /MACHINE:X64 2>&1"
