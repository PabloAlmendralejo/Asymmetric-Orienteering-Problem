...
// G_i, via bounded (n-1 round) Bellman-Ford in the (max,+) semiring --
// see solver_flow_torremocha.cpp for the full derivation.
static std::vector<double> compute_fatigue_bounds(const Input& inp, double rho) {
    ...
    for (int round = 0; round < n - 1; ++round) {
        ...
        for (const auto& a : arcs) {
            ...
            // solver_flow_torremocha.cpp's compute_fatigue_bounds comment
            // for why single-end-clip is unsound when psi can be negative
            // (rho>0); harmless no-op when rho=0 as in this paper's runs.
...
// Exact (clipped, sequence-dependent) fatigue-adjusted route cost.
...
struct LPModel {
    ...
    std::vector<int> t_col;                // legacy time variable -- kept for A-B comparison, not in fatigue budget anymore
    std::vector<std::vector<int>> w_col;   // legacy McCormick var for t_i*x_ij -- unused by new budget
    std::vector<int> G_col;                // asymmetric fatigue state G_i (replaces t_col in the budget)
    std::vector<std::vector<int>> u_col;   // McCormick var for G_i * x_ij
    ...
    void build(const Input& inp) {
        ...
        // Ghat_i bound (Bellman-Ford, max-plus, n-1 rounds).
        ...
        // constraint (add_fatigue_state_propagation) is one-sided (>=) and
        // nothing else in the model pressures G downward, so without this
        // the LP is free to settle on an inflated G at a partially-fixed
        // branch-and-cut node, making a genuinely feasible extension look
        // budget-infeasible there and pruning it away before it is ever
        // found. kGRegEps is chosen so it can never outweigh a single point
        // of objective value, so it only breaks ties among equally-good
        // solutions, toward the smallest feasible G.
        ...
        // NOTE: add_fatigue_budget (old, linear-in-time model) replaced by
        // add_fatigue_budget_asym below; kept defined, just not called.
    ...
    // McCormick envelope for u_ij ~= G_i * x_ij (mirrors add_mccormick).
    void add_fatigue_mccormick_asym() {
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
                ...
                // had +g_ub*x_ij with a 0.0 row lower bound (i.e. u_ij >=
                // G_i - g_ub*x_ij, missing the (1-x_ij) complement), which
                // forces u_ij >= G_i whenever x_ij=0 -- combined with
                // u_ij <= G_i below, that pins u_ij = G_i unconditionally,
                // and combined with u_ij <= g_ub*x_ij (=0 when x_ij=0)
                // forces G_i = 0 whenever ANY outgoing arc from i is unused,
                // i.e. on almost every real route. Found via a direct test:
                // fixing x to a known-feasible 40-node route made the LP
                // report infeasible even for a single-node prefix.
                add_row(-g_ub, 1e30,
    ...
    // sum C_ij x_ij + fatigue_rate * sum C_ij * u_ij <= B.
    // u_ij carries the unclipped conservative state G_i (>= true clipped
    // F_i), so this is always at least as tight as the true model -- see
    // solver_flow_torremocha.cpp comment for the Skorokhod argument.
...
int main(int argc, char** argv) {
    ...
    // (both solvers otherwise write the same op_output_torremocha_*.json names).
