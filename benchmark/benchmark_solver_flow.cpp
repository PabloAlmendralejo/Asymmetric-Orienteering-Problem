...
struct Input {
    ...
    std::vector<std::vector<double>> gain;   // phi_plus
    std::vector<std::vector<double>> loss;   // phi_minus
    ...
    void add_time_flow_coupling(const std::vector<std::vector<double>>& cm, double bud_raw, double fatigue_rate) {
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
                ...
                // Only when fatigue present (hurts no-fatigue instances)
                if (fatigue_rate > 0 && i > 0 && std::isfinite(cm[0][i]) && cm[0][i] > 1e-9) {
                    add_row(0.0, 1e30,
                            {f_col[i][j], x_col[i][j]},
                            {1.0,         -cm[0][i]});
                }
...
struct Solver {
    ...
    void process_node(BNCNode node, std::priority_queue<BNCNode>& node_pq) {
        ...
        for (int cut_iter = 0; cut_iter < max_cuts; ++cut_iter) {
            ...
            // Max-flow separation — only for no-fatigue instances. This is
            // the expensive one (observed: dominated a 13+ hour stall on a
            // n=150 instance) -- check the budget again right after it so a
            // single slow call can't be followed by several more before the
            // next check.
